...
class _AdminScreenState extends State<AdminScreen> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      final orgProvider = Provider.of<OrganizacionProvider>(context, listen: false);
      final depProvider = Provider.of<DepartamentoProvider>(context, listen: false);
                ...
                child: ListTile(
                  ...
                  subtitle: Column(
                    ...
                    children: [
                      ...
                      Text(u.emailUsuario, style: const TextStyle(fontSize: 12, color: Colors.grey)),
                        ...
                        children: [
                          Container(
                            ...
                            child: Text(
                              ...
                              style: TextStyle(
                                fontSize: 10,
                                color: rolNombre.toLowerCase() == 'admin' ? Colors.blue : Colors.green,
                  ...
                  trailing: PopupMenuButton<String>(
                    ...
                    onSelected: (value) {
                      if (value == 'editar') _mostrarFormulario(usuario: u);
                      else if (value == 'eliminar') _confirmarEliminar(context, u);
                    },
                    itemBuilder: (_) => [
                      const PopupMenuItem(
                        value: 'editar',
...
class _FormularioUsuarioState extends State<_FormularioUsuario> {
  ...
  Future<void> _guardar() async {
    ...
    if (_orgSeleccionada == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Selecciona una organización'), backgroundColor: Colors.red),
      );
      return;
    }
    ...
    bool ok;
    if (_esEdicion) {
      ok = await provider.actualizarUsuario(widget.usuarioEditar!.idUsuario!, usuario);
    } else {
      ...
      child: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            Row(
              children: [
                Expanded(child: _Campo(label: 'Nombre *', controller: _nombreCtrl, icono: Icons.person_outline)),
                const SizedBox(width: 12),
                Expanded(child: _Campo(label: 'Apellido *', controller: _apellidoCtrl, icono: Icons.person_outline)),
              ],
            ),
            ...
            TextField(
              ...
              decoration: InputDecoration(
                ...
                prefixIcon: const Icon(Icons.lock_outline, color: Color(0xFF185FA5), size: 20),
                suffixIcon: IconButton(
                  icon: Icon(_verPassword ? Icons.visibility_off : Icons.visibility, color: Colors.grey, size: 20),
                ...
                focusedBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(12), borderSide: const BorderSide(color: Color(0xFF185FA5), width: 2)),
              ),
            ),
            const SizedBox(height: 12),

            // ── Organización ─────────────────────────────────────────────
            _SelectorLabel(icono: Icons.business_outlined, label: 'Organización *'),
            ...
            // ── Departamento (filtrado por org) ──────────────────────────
            _SelectorLabel(icono: Icons.work_outline, label: 'Departamento'),
            const SizedBox(height: 8),
            ...
            // ── Rol ──────────────────────────────────────────────────────
            const Text('Rol *', style: TextStyle(fontSize: 13, fontWeight: FontWeight.w500, color: Colors.grey)),
            const SizedBox(height: 8),
            Row(
              children: [
                _RolChip(label: 'Usuario', seleccionado: _rolSeleccionado == 2, onTap: () => setState(() => _rolSeleccionado = 2)),
                const SizedBox(width: 10),
                _RolChip(label: 'Admin', seleccionado: _rolSeleccionado == 1, onTap: () => setState(() => _rolSeleccionado = 1), color: Colors.blue),
              ],
            ),
...
class _Campo extends StatelessWidget {
  ...
  const _Campo({required this.label, required this.controller, required this.icono, this.tipo = TextInputType.text});
...
class _RolChip extends StatelessWidget {
  ...
  const _RolChip({required this.label, required this.seleccionado, required this.onTap, this.color = Colors.green});
    ...
    return GestureDetector(
      ...
      child: AnimatedContainer(
        ...
        decoration: BoxDecoration(
          ...
          border: Border.all(color: seleccionado ? color : Colors.grey.shade300, width: seleccionado ? 2 : 1),
        ),
        child: Row(
          mainAxisSize: MainAxisSize.min,
          children: [
            Icon(seleccionado ? Icons.check_circle : Icons.radio_button_unchecked, size: 16, color: seleccionado ? color : Colors.grey),
            const SizedBox(width: 6),
            Text(label, style: TextStyle(color: seleccionado ? color : Colors.grey, fontWeight: seleccionado ? FontWeight.w600 : FontWeight.normal)),
          ],
        ),
      ),
    );
